		...
		<div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
			...
			<div class="container">
				<section>
					<div class="flex">
						<div class="max-w-xs">
							...
							<div class="mt-1 relative rounded-md shadow-md">
								<input type="text" name="wallet" id="wallet" v-model="ticker"
								@input="filterByInput(ticker)"
					...
					<button type="button" @click="add" @keydown.enter="add"
...
// 3. Количество запросов /Критичность 3
// 4. Запросы внутри компонента /Критичность 4
// 5. Магические строки и числа (URL, 5000 мс задержки, ключ локалстораж, количество на странице) / Критичность 4
// 5. Первый watch запускает второй 
// 6. Обработка ошибок API /Критичность 5
// 7. localStorage и анонимные вкладки
...
export default {
	...
	methods: {
		...
		filterByInput(){
			this.filter=[]
			let f= this.keys.filter(el=>{
				if(this.ticker)	return el.includes(this.ticker)
			})
			for(let i=0; i<4; i++){
				if(f[i]) this.filter.push(f[i])
				// console.log(this.ticker);
			}
		},
		matchAddedTickers(){
			if(this.ticker){
				this.addedTicker=this.tickers.some(el=> el.name===this.ticker)
			}else {
				this.addedTicker=false
			}
			// else this.addedTicker=false
		}
